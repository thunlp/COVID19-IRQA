...
from bertkpe.transformers import BertConfig, RobertaConfig, AdamW, WarmupLinearSchedule
logger = logging.getLogger()


config_class = {"bert-base-cased":BertConfig, "roberta-base":RobertaConfig}
...
class KeyphraseSpanExtraction(object):
    ...
    def __init__(self, args, state_dict=None):
        ...
        # -----------------------------------------------
        # select model
        network = networks.get_class(args.model_class)
        
        # -----------------------------------------------
        # select config
        if args.model_class in ['bert2tag', 'roberta2tag']:
    ...
    # --------------------------------------------------------
    # decode for bert2gram
    def test_bert2gram(self, inputs, lengths, max_phrase_words):
        self.network.eval()
        with torch.no_grad():
            logits = self.network(**inputs)
            logits = F.softmax(logits, dim=-1)
